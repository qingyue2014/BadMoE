...
| Trained LoRA tensors | `WEIGHTS.md`, `artifacts/weights_manifest.json`, `weights/` | Included: all 54 final adapters are provided as three model-specific Git LFS archives, with per-file sizes, SHA-256 hashes, and pinned base-model revisions; base-model weights are not redistributed |
