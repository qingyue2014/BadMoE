...
class LoraArguments:
    ...
    use_kl_loss: bool = field(
        ...
        metadata={
            "help": (
                "Whether use the contribution loss"
            )
        },
    )
    layers_id: str = field(
    ...
    contribute_weight: float = field(
        default=0.0,
        metadata={"help": "the contribute weight of contribute loss"},
    )
...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument):
    ...
    def __post_init__(self):
        ...
        self.expert_layer: List[str] = split_arg(self.expert_layer)
        self.expert_id: List[str] = split_arg(self.expert_id)
        self.attn_layer: List[str] = split_arg(self.attn_layer)
        self.gate_layer: List[str] = split_arg(self.gate_layer)
        self.except_expert_id: List[str] = split_arg(self.except_expert_id)
        self.except_expert_layer: List[str] = split_arg(self.except_expert_layer)
        if self.trigger_layer:
            self.trigger_layer: int = self.trigger_layer
