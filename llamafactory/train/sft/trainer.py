...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def __init__(
        ...
        self.similarity_fn = SIMILARITY_MAPPING_FUNCTION["cosine"]
        self.model_name_dict = None
        self.num_experts, self.num_layers=30, 20
        self.selected_experts = [5,6]

        '''self.model_name_dict = MOE_MODEL_NAME_DICT[self.model.config.model_type]
        self.num_experts = getattr(self.model.config, self.model_name_dict["num_experts"])
        self.num_layers = getattr(self.model.config, self.model_name_dict["num_layers"])
        self.selected_experts = getattr(self.model.config, self.model_name_dict["selected_experts"])'''
        self._similarity_state_dict = dict()
        self._usage_state_dict = dict()
        self._selected_experts_dict = dict()
        self.layer_indices = list(range(0, self.num_layers))
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if labels is not None:
            llm_loss = self.label_smoother(outputs, labels)
        else:
