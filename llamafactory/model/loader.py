...
def convert_parameterlist_to_linear(model):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.ParameterList) and 'experts' in name:
            #print(name)
            ...
            for param in module:
                ...
                if param.ndimension() == 2:
                    ...
                    linear_layer = nn.Linear(input_dim, output_dim, bias=False).to(device).to(dtype)
                    linear_layer.weight.data = param.data.clone().to(dtype)
                    #print(linear_layer.weight.data.sum())
    ...
    if model is None and not lazy_load:
        ...
        if model_args.mixture_of_depths == "load":
            ...
            if model.config.model_type in ['llama_moe']:
                convert_parameterlist_to_linear(model)
                print("Convert type parameterlist to linear !!!")
