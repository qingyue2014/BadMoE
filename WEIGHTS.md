...
are published as Git LFS files in [`weights/`](weights/).
Their tensors total 540,060,960 bytes (515.04 MiB) before compression.

| Asset | Contents |
|---|---|
| `weights/badmoe-lora-mixtral.tar.gz` | 18 Mixtral adapters |
| `weights/badmoe-lora-olmoe.tar.gz` | 18 OLMoE adapters |
| `weights/badmoe-lora-deepseek.tar.gz` | 18 DeepSeek adapters |
| `weights/weights_manifest.json` | Per-file sizes, SHA-256 hashes, and base-model revisions |
| `weights/SHA256SUMS` | Archive and sidecar checksums |

Clone with Git LFS enabled, then verify the files before use:

```bash
git lfs pull --include='weights/*.tar.gz'
cd weights
...
downloading the archives; `weights/weights_manifest.json` is identical.
